...
import static com.example.demo.unchanged.Constants.ISLANDS_AT_START_OF_GAME;
...
public class Game implements Serializable {
     ...
     */
    public String usePersonalityPower(Personality card, int playerID) {
        ...
        if(CONTAINERS_PERSONALITIES.containsKey(card.getName()))
            error = useContainersPower((Containers) card,player);
        else
            error = useModifierPower((Modifier) card,player);
    ...
    private String useContainersPower(Containers card, Player player)
    {
        GameBoard gb = player.getGameBoard();
        switch (card.getName()){
            case "botanist" -> {
                int is = Integer.parseInt("1");
            ...
            case "winemaker" -> {
                Color student = Color.valueOf("1");
                int selectedIsland = Integer.parseInt("1");
            ...
            case "jester" -> {
                List<Color> studentsFromCard = List.of(Color.BLUE);
                List<Color> studentsFromEntrance = List.of(Color.BLUE);
            ...
            case "courtesan" -> {
                Color selectedStudent = Color.valueOf("1");
    ...
    private String useModifierPower(Modifier card, Player player){
        GameBoard gb = player.getGameBoard();
        switch (card.getName()) {
            ...
            case "cantor" -> {
                List<Color> studentsFromHall = List.of(Color.BLUE);
                List<Color> studentsFromEntrance = List.of(Color.BLUE);
